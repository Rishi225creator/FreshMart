...
public class FetchProductDescriptionServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
     
    public FetchProductDescriptionServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("fruit".equalsIgnoreCase(type)){
			...
			rd=request.getRequestDispatcher("admin_fruit_stock.jsp");
			rd.forward(request, response);
		}else if((type).equalsIgnoreCase("vegetable")){
			request.setAttribute("allProd", allProd);
			rd=request.getRequestDispatcher("admin_vegetable_stock.jsp");
			rd.forward(request, response);
		}
	}
